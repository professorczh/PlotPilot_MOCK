...
import { Bot, ChevronRight, Send, Terminal, Activity, CheckCircle2, Wand2, Sparkles, Loader2, X } from 'lucide-react';
...
export default function AgentCoreView({ 
  ...
  return (
    <div className="h-full hud-panel flex flex-col overflow-hidden border-hud-border/40">
      ...
      <div className="p-2.5 px-3 border-b border-hud-border/30 flex items-center justify-between shrink-0 bg-gradient-to-r from-brand-red/[0.02] to-transparent">
        <div className="flex items-center gap-3">
          <div className="relative group">
            ...
            <div className="relative w-9 h-9 bg-[#1a1a1a] flex items-center justify-center rounded-xl border border-brand-red/30 shadow-[inset_0_0_10px_rgba(220,38,38,0.2)]">
              <Bot className="w-5 h-5 text-brand-red" />
              <div className="absolute -top-0.5 -right-0.5 w-2 h-2 bg-green-500 rounded-full border-2 border-[#1a1a1a]" />
            </div>
          </div>
          <div>
            <h3 className="text-sm font-display font-bold text-text-main tracking-[0.2em] uppercase">墨枢</h3>
        ...
        <div className="flex items-center gap-1">
          <button 
            className="p-2 hover:bg-white/5 transition-colors group rounded-lg"
            title="张力心电图"
          >
            <Activity className="w-4 h-4 text-muted-text group-hover:text-brand-red transition-colors" />
          </button>
          {onClose && (
            <button 
              onClick={onClose}
              ...
              title="收起对话"
            >
              <ChevronRight className="w-5 h-5 text-muted-text group-hover:text-text-main" />
            </button>
          )}
        </div>
      </div>
      ...
      >
                  ...
                  <motion.div 
                    ...
                    className={cn(
                      ...
                      msg.role === 'user' 
                        ...
                        : cn(
                            "pt-2.5 px-3 pb-1.5 rounded-2xl relative group/ai-msg border",
                            isDarkMode ? "bg-panel-bg/40 border-hud-border/40" : "bg-slate-100/80 border-slate-200"
                          )
                    )}
                  >
                    {msg.role === 'ai' && (
                       <div className="flex items-center gap-2 mb-2 opacity-60 group-hover/ai-msg:opacity-100 transition-all duration-500">
                         <div className="relative w-5 h-5 flex items-center justify-center">
                            ...
                            <div className="relative w-full h-full bg-[#1a1a1a] border border-brand-red/40 flex items-center justify-center rounded-md shadow-sm">
        ...
        {(status === 'panel_open' || status === 'starting') && (
          <motion.div 
            ...
            className={cn(
              "p-3.5 border border-hud-border/20 rounded-xl relative overflow-hidden mt-1.5 group",
              isDarkMode 
          ...
          >
            ...
            {onCancelProcess && (
              <button 
                ...
                className="absolute top-3 right-3 p-1.5 text-muted-text/40 hover:text-text-main hover:bg-white/5 border border-transparent hover:border-hud-border/20 rounded-lg transition-all z-20"
              >
            ...
            <div className="flex flex-col items-center text-center gap-1.5 py-0.5">
              ...
              >
                ...
                <div className="relative w-9 h-9 bg-[#1a1a1a] border border-hud-border/30 flex items-center justify-center rounded-lg shadow-md">
      ...
      {/* Chat Input */}
      <div className="p-2 border-t border-hud-border/30 bg-panel-bg/20 shrink-0">
        <div className="mb-1.5">
          <ProcessMonitor 
            isThinking={activeTraceSteps.some(s => s.status === 'thinking')}
            isCompleted={activeTraceSteps.every(s => s.status === 'completed')}
        ...
        <form onSubmit={handleSubmit} className="relative">
          <input 
            ...
            className="w-full bg-app-bg/50 border border-hud-border/50 pl-4 pr-12 py-2 text-sm focus:outline-none focus:border-brand-red/50 transition-all font-sans rounded-xl"
          />
          <button 
            type="submit"
